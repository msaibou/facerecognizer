...
import { createFaceSet, searchFace } from '../API/faceplusplus';
...
class Scan extends Component {
    ...
    takePicture = async function () {
        if (this.camera) {
            ...
            searchFace(this.faceset, data)
                .then(result => {
                    //console.log(result);
                    if (result.results) {
                        console.log("Here the results : ", result.results[0].confidence, result.results[0].face_token);
                        firestore()
                            .collection('users')
                            .where('face_token', 'array-contains-any', [result.results[0].face_token])
                            .get()
                            .then(querySnapshot => {
                                if (querySnapshot.size > 0) {
                                    const infUser = querySnapshot.docs[0].data();
                                    alert("C'est " + infUser.nom + " " + infUser.prenom + " à " + result.results[0].confidence + " %");
                                } else {
                                    alert("Le token [" + result.results[0].face_token + "] est introuvable sur la base !");
                                }

                            });
                    } else {
                        alert("Visage non trouvé!!");
                    }

                })
                .catch(error => console.log("Erreur lors de la recherche face++ : " + error));
